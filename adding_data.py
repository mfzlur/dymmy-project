""""
Run `python3 adding_data.py` to add the data
"""
...
with app.app_context():
    ...
    if not Assignment.query.filter_by(week=1, course_id=course1.id).first():
        assignment1 = Assignment(week=1, course_id=course1.id, due_date=datetime(2025, 5, 5))
        db.session.add(assignment1)

    if not Assignment.query.filter_by(week=2, course_id=course1.id).first():
        assignment2 = Assignment(week=2, course_id=course1.id, due_date=datetime(2025, 5, 12))
        db.session.add(assignment2)

    if not Assignment.query.filter_by(week=3, course_id=course1.id).first():
        assignment3 = Assignment(week=3, course_id=course1.id, due_date=datetime(2025, 5, 19))
        db.session.add(assignment3)

    if not Assignment.query.filter_by(week=4, course_id=course1.id).first():
        assignment4 = Assignment(week=4, course_id=course1.id, due_date=datetime(2025, 5, 26))
        db.session.add(assignment4)
